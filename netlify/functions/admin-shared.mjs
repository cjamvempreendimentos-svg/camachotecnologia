...
const adminStore = (context) => isProduction(context)
  ? getStore('camacho-admin', { consistency: 'strong' })
  : getDeployStore('camacho-admin-preview');
const metricsStore = (context) => isProduction(context)
  ? getStore('camacho-metrics', { consistency: 'strong' })
  : getDeployStore('camacho-metrics-preview');
...
export async function login(password, context) {
  ...
  await adminStore(context).setJSON(`session:${token}`, { expiresAt: new Date(Date.now() + 8 * 3600000).toISOString() });
  return { token };
}
...
export async function requireAdmin(req, context) {
  ...
  const session = await adminStore(context).get(`session:${token}`, { type: 'json' });
  if (!session || new Date(session.expiresAt) < new Date()) return null;
  return session;
}

export async function addMetric(data, context) {
  ...
  await metricsStore(context).setJSON(`metric:${Date.now()}:${randomUUID()}`, {
    event: data.event,
...
export async function listMetrics(days = 30, context) {
  const cutoff = Date.now() - Math.min(90, Math.max(1, Number(days)||30)) * 86400000;
  const s = metricsStore(context);
